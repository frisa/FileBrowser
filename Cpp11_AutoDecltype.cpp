...
/*
    ...
    <type-constraint> decltype(auto)    : type is deduced from the expression as initializer
...
/*
    ...
    has to be specified. When possible, the compiler will deduce the missing template arguments from the function arguments. 
    This occurs when a function call is attempted, when an address of a function template is taken, and in some other contexts

    This mechanism makes it possible to use template operators, since there is no syntax to define template arguments for an operator
*/
...
void Cpp11_AutoDecltype::auto_SimpleTypeDeduction()
{
    ...
    static_assert(std::is_same<decltype(auto_variable), int>::value, "variable does not have expected type");

    int type_variable1{0};
    auto auto_variable1 {type_variable1};
